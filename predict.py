...
class DatasetInference:
    def __init__(self, json_filepath: Union[str, Path], db_path: str = None) -> None:
        self.json_filepath = json_filepath
        self.ds = self.load_json_file()
        self.db_path = db_path if db_path else Path(self.json_filepath).parent
    ...
    def load_sample(self, index):
        ...
        Args:
            index: int of sample index.
        Returns:
            imagepath: Path to image.
            item_type: str definining item_type.
            convo: prompt and target description for this item."""
        data = self.ds[index]
    ...
    def load_and_pprint_sample(self, full=False, print=True):
        """Debugging method to print item description, plus all filtered elements that are kept from the training data.
        ...
        if print:
            if full:
                ...
                print(
                    f'{data["debug_info"]["title"]}, from: {data["debug_info"]["vendor"]}'
                )
                print(f'Inferred type: {data["debug_info"]["type_inference"]["type"]}')
                print(
                    f'Matching key: {data["debug_info"]["type_inference"]["match_key"]}'
                )
                print(
